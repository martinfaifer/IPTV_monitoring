...
class TsDuckAnalyzeAction
{
    public function execute(string $streamUrl)
    {
        if (str_contains($streamUrl, 'http')) {
            return shell_exec("tsp -I http --connection-timeout 200 {$streamUrl} -P until -s 1 -P analyze --json -O drop");
        }

        return shell_exec("tsp -I ip {$streamUrl} -P until -s 1 -P analyze --json -O drop");
    }
