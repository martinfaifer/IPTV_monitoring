...
class TsDuckAnalyzeAction
{
    public function execute(string $streamUrl)
    {
        if (str_contains($streamUrl, 'http')) {
            return shell_exec("timeout " . config('app.process_tsduck_timeout') . "s tsp -I http {$streamUrl} -P until -s 2 -P analyze --json -O drop");
        }

        return shell_exec("timeout " . config('app.process_tsduck_timeout') . "s tsp -I ip {$streamUrl} -P until -s 2 -P analyze --json -O drop");
    }
