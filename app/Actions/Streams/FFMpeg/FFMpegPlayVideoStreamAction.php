...
class FFMpegPlayVideoStreamAction
{
    public function execute(object $stream): void
    {
        if (str_contains($stream->stream_url, 'http')) {
            $command = "ffmpeg -i {$stream->stream_url} -c:v h264_qsv -b:v 2000k -c:a aac -strict experimental -b:a 128k " . public_path("storage/videos/{$stream->id}.m3u8");
            ...
            $command = "ffmpeg -i udp://{$stream->stream_url} -c:v h264_qsv -b:v 2000k -c:a aac -strict experimental -b:a 128k " . public_path("storage/videos/{$stream->id}.m3u8");
            // ffmpeg -i {$stream->stream_url} -c:v h264_nvenc -b:v 2000k -c:a aac -strict experimental -b:a 128k public_path("storage/videos/{$stream->id}.m3u8")
        }
